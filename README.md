...
```
nix-shell https://github.com/input-output-hk/jormungandr-nix/archive/<commit>.tar.gz
```
Once in the shell run `run-jormungandr` to start jormungandr.

You can tweak the blockchain parameter through nix-shell parameters, eg.:
```
nix-shell --arg faucetAmounts "[ 100000 1234444 34556 ]" \
          --arg numberOfStakePools 2 \
          --argstr consensusMode bft \
          --argstr storage "/tmp/jormungandr-storage" \
          --argstr rootDir /tmp/test \
...
| `--arg linear_fee_constant` | `10` | [1] |
| `--arg linear_fee_coefficient` | `0` | [1] |
| `--arg linear_fee_certificate` | `0` | [1] |
